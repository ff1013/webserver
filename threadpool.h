...
private:
    ...
    bool m_stop;
public:
    threadpool(int thread_number=8,int max_requests =100000);//线程数量，最大等待请求数量
    ~threadpool();
...
//线程池构造函数
template<typename T>
threadpool<T>::threadpool(int thread_number,int max_requests)://：后可以对成员进行初始化
