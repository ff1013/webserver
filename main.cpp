...
void addsig(int sig,void(handler)(int)){
    ...
    sigfillset(&sa.sa_mask);//临时阻塞的信号集
...
int main(int argc,char* argv[]){
    ...
    //获取端口号
    int port=atoi(argv[1]);//字符串转换成数字

    //对SIGPIPE信号进行处理
    addsig(SIGPIPE,SIG_IGN);//ignore忽略
    ...
    address.sin_port=htons(port);//字节序转换
